...
# 给识别一句简体中文「定调」提示：强制输出简体（否则 Whisper 常吐繁体）+ 略微提升连贯度。
# 也可在这里塞常用词/人名帮助识别，如「贾维斯、阿树、deepseek」。
ASR_INITIAL_PROMPT = os.environ.get(
    "JARVIS_ASR_PROMPT", "以下是简体中文普通话的语音内容。")
...
FRAME_MS = 30                     # 每帧时长
SILENCE_TAIL = 0.5                # 句尾静音多久判定说完（秒）；0.7→0.5 提速约 200ms，太低易把停顿当说完
